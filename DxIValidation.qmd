...
if (Sys.info()["sysname"] == "Linux") {
...
} else if (file.exists("I:\\Institut-Haus 04\\")) {
 ...
 # # to read csv files in batch:
 # 
 # all.data <- list()
 # #loop through all files
 # for (i in 1:length(csv.files)) {
 #   val.dat <- read_csv(csv.files[i])
 #   val.dat <- val.dat |>
 #     rename(
 ...
 #   all.data[[i]] <- val.dat
 # }
 # val.dat <- rbindlist(all.data, use.names = TRUE, fill = TRUE)
...
# dbExecute(con, "DELETE FROM TranslationData")
