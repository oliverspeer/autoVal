...
# Combined custom distance function
adjust_for_key_terms_and_abbreviation <- function(string1, string2, method = "osa", key_terms = c("t3")) {
  base_distance <- stringdist::stringdist(tolower(string1), tolower(string2), method = method, nthread = 4)
  
  # Adjust for key terms
  for (term in key_terms) {
    if (grepl(term, tolower(string1)) && grepl(term, tolower(string2))) {
      base_distance <- base_distance * 0.75 # Adjusting the score for key term presence
    }
  }
