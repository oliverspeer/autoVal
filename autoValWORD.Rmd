...
## Pearson correlation
text(paste("Pearson's r:", round(cor(method.x, method.y), 2)), x=max(method.x)*0.8, y=max(method.y)*0.3)
...
text(cex=.65,x=max(validation.data)*0.8,y=max(validation.data)*0.37,sprintf("Gradient : %4.2f (%4.2f - %4.2f)", gradient_est, gradient_lci, gradient_uci))
text(cex=.65,x=max(validation.data)*0.8,y=max(validation.data)*0.3,sprintf("Intercept : %4.2f (%4.2f - %4.2f)", intercept_est, intercept_lci, intercept_uci))
text(cex=.65,x=max(validation.data)*0.8,y=max(validation.data)*0.23, sprintf("Alpha : %4.2f", alpha_ci))
...
plotBias(m2, ci.area=TRUE, ci.border=TRUE, prop=TRUE, cex.sub=0.5)
```
