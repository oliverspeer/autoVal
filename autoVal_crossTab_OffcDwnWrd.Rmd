...
```{r Kreuttabelle, echo=FALSE, warning = FALSE, message = FALSE}
library(dplyr)



# Define breaks and labels for categories
area <- c(-Inf, 300, 600, Inf) # define the breaks
cutoff <- c("<300", "300-600", ">600") # define the labels

# Categorize 'BNII' and 'AU5800' using cut
data <- validation.data %>%
  mutate(
    Category_BNII = cut(BNII, breaks = area, labels = cutoff, right = FALSE),
    Category_AU5800 = cut(AU5800, breaks = area, labels = cutoff, right = FALSE)
  )

# Create crosstabs
crosstab_BNII <- data.frame(table(data$Category_BNII))
crosstab_AU5800 <- data.frame(table(data$Category_AU5800))

combined_data <- merge(crosstab_BNII, crosstab_AU5800, by.x = "Var1", by.y = "Var1", suffixes = c(" BNII", " AU5800"))
names(combined_data)[1] <- "Category"

combined_data |>  flextable() |> 
   bold(i = 1) |> 
  theme_vanilla() |>   
   bg(bg = "darkgrey", i = c(1,3)) |> 
  # bg(bg = "lightgrey", i = c(11, 15, 17)) |> 
  set_table_properties(width = 0.8, layout = "autofit") |> 
  colformat_date(fmt_date = "%d/%m/%Y")
